...
class BitstringHamiltonian:
    ...
    one_electron_integrals
        Array ``h[p, q]`` over spatial orbitals.
    two_electron_integrals
        Array ``g[p, q, r, s]`` in chemist notation ``(pq|rs)``.
    nuclear_repulsion
        Optional constant energy shift added to diagonal matrix elements.

    Notes
    -----
    Determinants are stored as separate alpha and beta bitstrings over spatial
    orbitals. Internally, operator phases use blocked spin-orbital ordering:
    ``alpha(0..n-1), beta(0..n-1)``.
    """
